...
# modified siren layer
class QResSiren(nn.Module):
    def __init__(self, dim_in, dim_out, w0=1., is_first=False, use_bias=True, activation=None):
        super().__init__()
        self.dim_in = dim_in
        self.is_first = is_first
        
        self.qres = torch.nn.QResLayer(dim_in, dim_out, use_bias)
...
torch.nn.QResSiren = QResSiren


# Helge Mohn (2021)
class DeepNeuralNet(pl.LightningModule):
    def __init__(self, hparams, w0=1., w0_initial=30., use_bias=True, final_activation=None, *args, **kwargs):
        super().__init__()
        ...
        for ind in range(hparams.num_layers-1):
            is_first = ind == 0
            ...
            # =================== Batchnorm ====================
            if normalize:
                layers.append(nn.BatchNorm1d(layer_dim_in))
            
            # =================== QResSiren ====================
            layers.append(QResSiren(
        ...
        # =================== Batchnorm ====================
        if normalize:
            layers.append(nn.BatchNorm1d(self.num_neurons))
